...
function findSymptoms(text) {
  ...
  const terms = {
    fits: ["fit", "convuls", "seizure"], bleeding: ["bleed", "blood"], headache: ["headache", "head pain"],
...
export default function App() {
  ...
  return <><div className="topbar"><div className="brand">SafeMatri<small>Every mother heard. Risks caught early, anywhere.</small></div><div className="seg"><button aria-pressed={view === "woman"} onClick={() => setView("woman")}>Woman's app</button><button aria-pressed={view === "worker"} onClick={() => setView("worker")}>Health worker dashboard</button></div></div><div className="notice">Prototype for demonstration only. Not a medical device. Do not use it for real medical decisions.</div><main>{view === "woman" ? <WomanView onDashboard={() => setView("worker")} /> : <WorkerView />}</main><footer>SafeMatri prototype by Team Syrah · All cases are fictional demo data and SMS alerts are simulated.</footer></>;
}